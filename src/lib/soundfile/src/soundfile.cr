...
module SoundFile
  class SFile
    ...
    def set_string(str, type)
      ...
      @handle.try { |handle| LibSndFile.set_string(handle, type, str) }
    end

    def get_string(type)
      type = LibSndFile::StringType.parse(type).value
      str = @handle.try { |handle| LibSndFile.get_string(handle, type) }
      return "" unless str
      String.new(str)
    end
    ...
    def seek(frames, whence)
      ...
      @handle.try { |handle| LibSndFile.seek(handle, frames, whence) }
    end
    ...
    def error_number
      @handle.try { |handle| LibSndFile.error(handle) }
    end

    def error_to_s
      error_number.try { |number| String.new(LibSndFile.str_error(number)) }
    end
    ...
    def sf_version
      ...
      @handle.try { |handle| LibSndFile.command(handle, cmd, ver, ver.size) }
    ...
    def get_log_info
      ...
      size = @handle.try { |handle| LibSndFile.command(handle, cmd, log, log.size) }
      return "" if size == 0
    ...
    def calc_max_all_channels
      ...
      return nil unless @handle.try { |handle| LibSndFile.command(handle, cmd, max, sizeof(Float64) * channels) } == 0
      max.to_a
    end
    ...
    def get_signal_max
      ...
      return nil unless @handle.try { |handle| LibSndFile.command(handle, cmd, max_ptr, sizeof(Float64)) } == 1
      max
    end
    ...
    def get_max_all_channels
      ...
      return nil unless @handle.try { |handle| LibSndFile.command(handle, cmd, max, sizeof(Float64) * channels) } == 1
      max.to_a
    end
    ...
    def set_norm_float(val)
      ...
      @handle.try { |handle| LibSndFile.command(handle, cmd, nil, true_false) }
    end
    ...
    def get_simple_format_count
      ...
      return nil unless @handle.try { |handle| LibSndFile.command(handle, cmd, count_ptr, sizeof(Int32)) } == 0
      count
    end
    ...
    def get_simple_format(fmt = 9)
      ...
      return nil unless @handle.try { |handle| LibSndFile.command(handle, cmd, ptr, size) } == 0
      format_info
    end
    ...
    def get_format_info(format_info : LibSndFile::SFFormatInfo)
      ...
      @handle.try { |handle| LibSndFile.command(handle, cmd, ptr, sizeof(LibSndFile::SFFormatInfo)) }
      format_info
    end
    ...
    def get_format_major_count
      ...
      raise "format_major_count error" unless @handle.try { |handle| LibSndFile.command(handle, cmd, count_ptr, sizeof(Int32)) } == 0
      count
    end
    ...
    def get_format_subtype_count
      ...
      raise "format_subtype_count error" unless @handle.try { |handle| LibSndFile.command(handle, cmd, count_ptr, sizeof(Int32)) } == 0
      count
    end
    ...
    def get_embed_file_info
      ...
      @handle.try { |handle| LibSndFile.command(handle, cmd, ptr, sizeof(LibSndFile::SFEmbedFileInfo)) }
    end
    ...
    def wavex_set_ambisonic(ambi)
      ...
      res = @handle.try { |handle| LibSndFile.command(handle, cmd, nil, val.value) }
      return "SF_AMBISONIC_NONE" if res == LibSndFile::Ambisonic::SF_AMBISONIC_NONE.value
      return "SF_AMBISONIC_B_FORMAT" if res == LibSndFile::Ambisonic::SF_AMBISONIC_B_FORMAT.value
      res
    end
    ...
    def set_vbr_encoding_quality(quality : Float64)
      ...
      @handle.try { |handle| LibSndFile.command(handle, cmd, quality_ptr, sizeof(Float64)) }
    end
    ...
    def set_compression_level(level : Float64)
      ...
      @handle.try { |handle| LibSndFile.command(handle, cmd, level_ptr, sizeof(Float64)) }
    end
    ...
    def get_cue_count
      ...
      res = @handle.try { |handle| LibSndFile.command(handle, cmd, pointerof(count), sizeof(UInt32)) }
